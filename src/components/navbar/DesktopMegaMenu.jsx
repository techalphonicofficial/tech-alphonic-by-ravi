...
export default function DesktopMegaMenu({ item, open }) {
          ...
          <div className="mb-7 flex items-end justify-between border-b border-dark/10 pb-5">
            ...
            <Link
              href={`/services/${item.label
                ...
                .replace(/\s+/g, "-")
                .replace(/&/g, "and")}`}
              className="group hidden items-center gap-2 rounded-full border border-primary/20 px-4 py-2 text-xs font-semibold text-primary transition-all duration-300 hover:border-primary hover:bg-primary hover:text-white lg:flex"
            >
              View All Services
                ...
                <div className="space-y-1">
                  {column.items?.map((link, linkIndex) => (
                    <Link
                      ...
                      href={link.href}
                      className="group/link relative flex items-center justify-between rounded-xl px-3 py-2.5 text-[13px] font-medium text-text transition-all duration-300 hover:translate-x-1 hover:bg-primary/5 hover:text-primary"
                    >
